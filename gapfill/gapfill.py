...
def main():
    ...
    p2.add_argument("--model-out", help="Output model file path for phase2")
    ...
    p3.add_argument("--model-out", help="Output model file path for phase3")
    ...
    p3.add_argument(
        ...
        default="exact",
        help="Phase3 strategy for blocked mode (ignored for deadends)",
    )
    p3.add_argument(
        "--solver-lp",
        choices=["glpk", "glpk_exact", "scipy", "gurobi", "cplex"],
        default="glpk",
        help="LP solver for FBA (sink_milp only). gurobi/cplex require licenses.",
    )
    # sink_milp specific options for phase3
    p3.add_argument(
        "--components",
        type=str,
        default=None,
    ...
    pall.add_argument("--model-out", help="Output model file path for phase2")
    ...
    if args.cmd == "phase1":
        ...
        model = args.model or os.path.join(
            ...
            "models",
            "base",
            "THG-beta-batch_251106_unconnected.json",
        )
    ...
    if args.cmd == "phase2":
        ...
        if args.mode == "minimal":
            ...
            out_csv, model_out, n = run_phase2(
                cand,
                unconnected_model_json=args.model,
                out_dir=args.out,
    ...
    if args.cmd == "phase3":
        ...
        if args.mode == "deadends":
            ...
            if args.strategy == "tiered_milp":
                ...
                out_csv, model_out, n = ph3_tiered.run_phase3_tiered(
                    cand,
                ...
                print(f"Phase3 tiered MILP wrote {out_csv}, model {model_out}, n={n}")
            elif args.strategy == "component_milp":
                print("Running Phase-3 (Component-wise MILP optimizer)...")
                ph3_cmilp = importlib.import_module("phase3_component_milp")
                components_csv = os.path.join(
                    args.out or os.path.join(os.path.dirname(__file__), "files"),
                    "components_summary.csv",
                )
                out_csv, model_out, n = ph3_cmilp.run_phase3_component_wise(
                    cand,
                    starting_model_json=args.model,
                    components_csv=components_csv,
                    out_dir=args.out,
                ...
                print(
                    f"Phase3 component MILP wrote {out_csv}, model {model_out}, n={n}"
                )
            elif args.strategy == "milp":
                print("Running Phase-3 (Global MILP optimizer)...")
                ph3_milp = importlib.import_module("phase3_milp_optimizer")
                out_csv, model_out, n = ph3_milp.run_phase3(
                    cand,
                    starting_model_json=args.model,
                    out_dir=args.out,
                    phase3_model_out=args.model_out,
                ...
                print(f"Phase3 MILP wrote {out_csv}, model {model_out}, n={n}")
            elif args.strategy == "sink_milp":
                print("Running Phase-3 (Sink MILP strategy)...")
                print(
                    "Note: sink_milp requires unconnected model - using starting model as both phase2 and unconnected"
                )
                ...
                result = ph3_sink.run_phase3_all_components(
                    cand,
                    phase2_model_json=args.model,
                    unconnected_model_json=args.model,  # Using same model for both
                    out_dir=args.out
                    or os.path.join(os.path.dirname(__file__), "files"),
                    phase3_model_out=args.model_out,
                ...
                out_csv, model_out, n = ph3_block.run_phase3(
                    cand,
                    starting_model_json=args.model,
                    out_dir=args.out,
                    max_additions=args.max,
