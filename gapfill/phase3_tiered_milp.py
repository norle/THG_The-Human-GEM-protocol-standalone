...
def filter_candidates_by_type(candidates, candidate_type):
    ...
    return [
        c for c in candidates if c.get("type", "").upper() == candidate_type.upper()
    ]
...
def process_tier_for_component(
    model, tier_candidates, blocked_rxns, tradeoff_lambda, verbose=True
):
    ...
    coverage = compute_coverage_for_component(
        model, tier_candidates, blocked_rxns, verbose
    )

    # Filter to candidates with any coverage
    effective = [
        c for c in tier_candidates if coverage.get((c["met1"], c["met2"]), set())
    ]
...
def run_phase3_tiered(
    ...
    for comp_idx, (comp_id, comp_size) in enumerate(isolated_comps):
        ...
        for tier_name, tier_candidates in [("A", type_a), ("B", type_b), ("C", type_c)]:
            ...
            comp_tier_cands = get_candidates_for_component(
                tier_candidates, comp_id, used_candidates
            )
            ...
            if verbose:
                print(
                    f"    Added {len(selected)} PTRs, unblocked {unblocked_count} reactions"
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Tiered MILP Phase-3 optimizer (A → B → C)"
    )
