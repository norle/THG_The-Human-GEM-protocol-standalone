...
if __name__ == '__main__':
    ...
    input_model_path = os.path.join(project_root, "models", "THG-2023-02-25.xml")
    model = read_sbml_model(input_model_path)

    variablesFile = os.path.join(project_root, "files", "variablesFile.pkl") 
    Output = os.path.join(project_root, "models", "model_bb.xml")  
    Output2 = os.path.join(project_root, "files", "Rxn2Fix")
    Output3 = os.path.join(project_root, "models", "THG-beta2-tmp.xml")  
    Output4 = os.path.join(project_root, "models", "THG-beta2.xml")  
    Output5 = os.path.join(project_root, "models", "THG-beta1.xml")  

    handle = open(Output2, 'w')

    #Parameters about the compartments (change according to your model)
    excel_file = os.path.join(project_root, "files", "ListOfCompartments_sept2024.xlsx") # Excel file containing the compartments information
    ...
    column_index = 0  # Column with the compartment mapping corresponding to your desired model 
    abbreviations_sheet_name = 'Endo1a_abb'  # Sheet name containing the abbreviations of the compartments
    ...
    create_compartments_dict_bm(excel_file, compartments_sheet_name, column_index, location_pkl_file) #create the pkl file: dict_compartments.pkl

    #Create a dictionary with the abbreviations of the compartments as keys and the full names as values
    comp_abb_file = os.path.join(project_root, "files", "compartments_abbreviations.pkl")
    create_comp_abbreviations_dict_bm(excel_file, abbreviations_sheet_name, comp_abb_file) #create the pkl file: compartments_abbreviations.pkl
    comp_dict = pd.read_pickle(comp_abb_file)

    c , comp_dict = compartment_file_to_dict_bm(excel_file,compartments_sheet_name,comp_dict, comp_abb_file) #c is the mapping with the full name as keys, abbreviations as values and comp_dict is the mapping with the abbreviations as keys and the full name as values
                    ...
                    for ec in EC:
                        ...
                        if not ec in variables:
                            ...
                            if new_gpr[-1]:
                                ...
                                for key, value in new_gpr[-1].items():  #separate the complete gpr into individual gprs
                                    gpr=value
                                    print("gpr: ", gpr)
                                    ...
                                    new_locations = getLocation(gpr, genelist1, genelist2, 1, location_pkl_file, session)
                        ...
                        for CSL in [*variables[x2.id][0]]:                        
                            ...
                            if not ID in [rxn[1] for rxn in Rxn[re.sub('[a-z]+[0-9]*','', x2.reaction)]]:
                                ...
                                if 'sGPR' in reaction2.annotation: 
                                    print("before adding sGPR: ", reaction2.annotation['sGPR'])

                                try:
                                    reaction2.annotation['sGPR'] = variables[x2.id][0][compartment_name]
                                except KeyError:
                                    reaction2.annotation['sGPR'] = ''
                                print("after adding sGPR: ", reaction2.annotation['sGPR']) 
                                ...
                                try:
                                    model2.reactions.get_by_id(x2.id).annotation['sGPR'] = variables[x2.id][0][compartment_name]
                                    print("after adding sGPR: ", variables[x2.id][0][compartment_name])
                                except KeyError:
                                    if 'sGPR' in model2.reactions.get_by_id(x2.id).annotation:
                                    ...
                                    print("after adding sGPR: (key error) ", model2.reactions.get_by_id(x2.id).annotation['sGPR'])
