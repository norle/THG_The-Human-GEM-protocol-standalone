...
| `thg_protocol.annotation.metabolites.generate_met_annotation` | Metabolite records, an explicit annotation output path, and an optional PubChem client | Returns annotated and unresolved records; writes annotation and failure reports beside the supplied output path. `checkpoint_interval`, `resume`, and `delay_between_requests` are active; an input-fingerprinted checkpoint sidecar is removed after successful finalization. |
| `thg_protocol.gpr.location.resolve_locations` | GPR, gene names/IDs, injected location and optional Ensembl clients, and an explicit fallback policy | Returns the historical four rule/mapping dictionaries. Boolean GPR structure is preserved; unmatched pages remain unresolved unless `fallback_location` is explicitly supplied. |
