...
def test_beta2_registered_workflow_exports_bundle_and_resumes(tmp_path):
    ...
    assert {item["role"] for item in export["outputs"]} == {
        "model", "sbml", "validation", "plan", "proposals", "id-registry", "ledger",
        "location-evidence", "diff", "unresolved", "summary", "provenance",
    }
    attempts = {stage: entry["attempt"] for stage, entry in status["steps"].items()}
    resume(run_dir)
    assert {stage: entry["attempt"] for stage, entry in get_status(run_dir)["steps"].items()} == attempts
    ...
    assert rerun["collect-location-evidence"]["attempt"] == attempts["collect-location-evidence"] + 1
...
def test_beta2_consumes_only_verified_beta1_export(tmp_path):
    ...
    beta1_config.write_text(json.dumps({
        "format_version": 2,
        "workflow": "beta1",
    ...
    payload["beta2"]["upstream"] = {
        "run_dir": str(beta1_run), "stage_id": "export-beta1", "role": "model"
    }
...
def test_sanctioned_beta1_to_beta2_candidate_passes_gate(tmp_path):
    ...
    beta1_config.write_text(json.dumps({
        "format_version": 2, "workflow": "beta1",
        "run": {"name": "sanctioned-beta1", "output_dir": str(beta1_run)},
        "beta1": {"input_model": str(fixture), "sanctioned_model": True, "run_solver_checks": True},
    }), encoding="utf-8")
    start(beta1_config)
    ...
    beta2_config.write_text(json.dumps({
        "format_version": 2, "workflow": "beta2",
        "run": {"name": "sanctioned-beta2", "output_dir": str(beta2_run)},
        "beta2": {
            "upstream": {"run_dir": str(beta1_run), "stage_id": "export-beta1", "role": "model"},
            "gene_locations": {"ENSG000001": ["cytosol"], "ENSG000002": ["mitochondria"]},
            "compartments": {"c": "cytosol", "m": "mitochondria"},
            "run_solver_checks": True,
        },
    }), encoding="utf-8")
    ...
    export_dir = beta2_run / __import__("pathlib").Path(status["steps"]["export-beta2"]["outputs"][0]["path"]).parent
