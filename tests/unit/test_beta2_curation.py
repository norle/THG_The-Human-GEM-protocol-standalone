...
def test_gpr_location_union_and_complex_intersection():
    assert resolve_gpr_locations("A or B", {"A": {"Mitochondria"}, "B": {"Cytosol"}}).rules == {
        "Cytosol": "B",
        "Mitochondria": "A",
    }
    result = resolve_gpr_locations("A and B", {"A": {"Cytosol", "Mitochondria"}, "B": {"Mitochondria"}})
    assert result.rules == {"Mitochondria": "(A and B)"}
    mixed = resolve_gpr_locations("(A and B) or C", {"A": {"Mitochondria"}, "B": {"Mitochondria"}, "C": {"Cytosol"}})
    ...
    conflict = resolve_gpr_locations("A and B", {"A": {"Cytosol"}, "B": {"Mitochondria"}})
...
def test_expansion_is_proposal_first_and_deterministic():
    ...
    plans = generate_expansion_plan(model, {"R1": {"Mitochondria": "G1"}}, compartments={"c": "Cytosol", "m": "Mitochondria"})
    assert len(plans) == 1
    ...
    assert generate_expansion_plan(model, {"R1": {"Mitochondria": "G1"}}, compartments={"c": "Cytosol", "m": "Mitochondria"}) == plans
