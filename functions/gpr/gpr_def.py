...
def pattern_match_org_2(page: str, org: str = "Homo Sapiens") -> List[str]:
    ...
    matched_genes = sorted([
        match[0] for line in page.splitlines()
...
def replace_specific_and_with_or(gpr, pairs, d2):
    ...
    for gene1, gene2 in pairs:
        if check_genes_in_d2((gene1, gene2), d2):
            gpr = re.sub(rf'\[{gene1}\*\d+\*0\] and \[{gene2}\*\d+\*0\]', rf'[{gene1}*48*0] or [{gene2}*12*0]', gpr)
    return gpr
...
def replace_specific_and_with_or(gpr, pairs, d2):
    for gene1, gene2 in pairs:
        if check_genes_in_d2((gene1, gene2), d2):
            ...
            gpr = gpr[:match.start()] + gpr[match.start():match.end()].replace('and', 'or') + gpr[match.end():]
    return gpr
...
def update_urls3_with_coefficients(urls3, w):
    ...
    gene_dict = {item.split('*')[0]: item.split('*')[1] for sublist in w for item in sublist}
...
def parseGPRnewest(ec_number,
    urls0: List[str], page: str, session: requests.Session
) -> Tuple[List[str], str, str, str, str]:
    urls1 = [i for i in reversed(sorted([x[0:][0] for x in urls0], key=len))    ] # Sort genes by name lenght
    ...
    while iters < len(c):  # isoforms
        ...
        if re.findall(r'class="GENE"', c[iters]):
            ...
            if not d2:
                ...
                isopage = str(get_html(isourl[0].replace(" ", "").replace('"', ""), session))
            ...
            synonim = [
                multiple_replace(dict2, multiple_replace(dict1, x.strip().replace("&nbsp;", "")))
            ...
            if isinstance(d2, list):
                ...
                d2_list= d2.copy() #any modifications to d2_list will not affect the original d2 object, and vice versa.
                ...
                for d2 in d2_list:
                    ...
                    while j < len(d):
                        ...
                        while z < len(urls1):
                            ...
                            if IsVar:
                                ...
                                if not re.findall(r"]", htry) or not re.findall(r"\[", htry):
                                    htry = (
                                        "[" + str(htry) + "]*1"
                                ...
                                if re.findall(r"Subunit composition.*=", d1) or re.findall(
                                    ...
                                    NestLevel = re.findall(r"(?=(\]\*[0-9]+\]\*))", htry)
                                    #h0 = ParseNestedParen(str(htry), len(NestLevel))[0]
                                    h0=htry 
                                    Mult = re.findall(r"([\]\*0-9]+$)", htry) # list of all sequences at the end of the string htry that consist of any combination of closing square brackets ], asterisks *, and digits 0-9
                    ...
                    if reaction_id in rxn_gpr_dict:
                        ...
                        if gpr not in rxn_gpr_dict[reaction_id]:
                            ...
                            rxn_gpr_dict[reaction_id] = "[" + rxn_gpr_dict[reaction_id] + "] or [" + gpr + "]"
                    else:
                ...
                for y in x.split(" and "):  # Evaluate complex
                    if re.findall(r"[A-Za-z0-9].*?", y):  # eliminate empty spots
                        ...
                        if not aa[0][0] in dict4:
                            ...
                            dict4[aa[0][0]] = eval(str(dict4[aa[0][0]]) + "+" + aa[0][1])
    ...
    put_entries = {key: value for key, value in rxn_gpr_dict.items() if key.startswith('PUT')}
    ...
    rxn_gpr_dict = {**{key: value for key, value in rxn_gpr_dict.items() if not key.startswith('PUT')}, **put_entries}
