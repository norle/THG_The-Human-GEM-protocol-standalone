...
def UnwrapRxnSubsProdParam(Reaction, LibIni, IthRxnMB):
	...
	for x in SubsLibEnd: 
		if x in LibIni[0]:
			...
			if x == 'H':
				LibIni[0].update({x:[SubsLibEnd[x],'C00080']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'H2O':
				LibIni[0].update({x:[SubsLibEnd[x],'C00001']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'Fe':
	...
	for x in ProdLibEnd: 
		if x in LibIni[1]:
			...
			if x == 'H':
				LibIni[1].update({x:[ProdLibEnd[x],'C00080']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'H2O':
				LibIni[1].update({x:[ProdLibEnd[x],'C00001']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'Fe':
				LibIni[1].update({x:[ProdLibEnd[x],'C00023']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'Na':
				LibIni[1].update({x:[ProdLibEnd[x],'C01330']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'Ca':
				LibIni[1].update({x:[ProdLibEnd[x],'C00076']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'K':
				LibIni[1].update({x:[ProdLibEnd[x],'C00238']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'F':
				LibIni[1].update({x:[ProdLibEnd[x],'C00023']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'R':
				LibIni[1].update({x:[ProdLibEnd[x],'C00000']}) # This line has to be replaced by a function that gives the real ID
			elif x == 'X':
				LibIni[1].update({x:[SubsLibEnd[x],'C0000X']}) # This line has to be replaced by a function that gives the real ID
	return LibIni
...
def RxnParam2Eq(Reaction,MetList,MetEquiv):  
	Substrate = Reaction.Substrate()
	Product = Reaction.Product()
	gly_test = min([1 if x in list(MetEquiv.keys()) or x[0] == 'G' else 0 for x in [y[2] for y in Reaction.Substrate()+Reaction.Product()]]) # 1: All can be glycans, 0: at least 1 cannot be glycan
	c_test = min([1 if x in list(MetEquiv.keys()) or x[0] == 'C' else 0 for x in [y[2] for y in Reaction.Substrate()+Reaction.Product()]]) # 1: All can be compounds, 0: at least 1 cannot be compounds
	...
	if mb_test != 0:
		...
		elif c_test == 1:# if gly_test == 0:  # if str(Reaction.GTest()) != '1':
			s = 0
			S = ''
			while s < len(Substrate):
				SStch = str(Substrate[s][0])
...
def MB_Core(eq,AddH,H2O,RxnID):
	...
	if k and not BadKTest and TestI==0:
		...
		a = str(eval(str(list(filter(lambda p: not I in p, str(A).replace("[","").replace("]","").split(',')))).replace("'","").replace(" ","")))
		V = 1 # Variation
...
#		ListOfFunc = [CountAtom, MB_Core, MB_REM, MB_LP]
		...
		while not MB[0] and i < len(ListOfFunc)-1: 
			...
			if not MB[0] and not re.findall("H[^A-Z0-9]|[^A-Z0-9]H$|^[H][^A-Z0-9]|H1[^A-Z0-9]|[^A-Z0-9]H1$|^[H1][^A-Z0-9]",eq): # not H in the original equatio 					
				...
				if not MB[0]:                  
					...
					MB = ListOfFunc[i](eq.replace("->","-> H +"),AddH,AddH2O,RxnID)
					if MB[0]: eq=eq.replace("->","-> H +")
			if not MB[0] and not re.findall("H2O[^A-Z0-9]|[^A-Z0-9]H2O$|^[H2O][^A-Z0-9]|H2O1[^A-Z0-9]|[^A-Z0-9]H2O1$|^[H2O1][^A-Z0-9]",eq): # not H in the original equatio 					
			# if not MB[0] and not re.findall("H2O[^A-Z0-9]|[^A-Z0-9]H2O$|^[H2O][^A-Z0-9]",eq): # not H2O in the original equation				
				...
				if not MB[0]:                  
					...
					MB = ListOfFunc[i](eq.replace("->","-> H2O +"),AddH,AddH2O,RxnID)  
					if MB[0]: eq=eq.replace("->","-> H2O +")
			if not MB[0] and not re.findall("H[^A-Z0-9]|[^A-Z0-9]H$|^[H][^A-Z0-9]|H1[^A-Z0-9]|[^A-Z0-9]H1$|^[H1][^A-Z0-9]",eq) and not re.findall("H2O[^A-Z0-9]|[^A-Z0-9]H2O$|^[H2O][^A-Z0-9]|H2O1[^A-Z0-9]|[^A-Z0-9]H2O1$|^[H2O1][^A-Z0-9]",eq):               
...
#		NewSpecies = RxnCompare(eq,eq_init)
		if not MB[0]:
			i=len(ListOfFunc)-1
			MEtInEq = [x.split(' ')[-1] for x in [x.strip() for x in [x for x in eq.split('->')][0].split('+')]],[x.split(' ')[-1] for x in [x.strip() for x in [x for x in eq.split('->')][1].split('+')]]
			IsH = ([1 for x in MEtInEq[0] if x == 'H' or x == 'H1'],[1 for x in MEtInEq[1] if x == 'H' or x == 'H1'])
			IsH2O = ([1 for x in MEtInEq[0] if x == 'H2O' or x == 'H2O1'],[1 for x in MEtInEq[1] if x == 'H2O' or x == 'H2O1'])
			...
			eq = MB[2]
#		AllSpecies = ([x for x in [x.replace(" ","") for x in eq.split('->')][0].split('+')],[x for x in [x.replace(" ","") for x in eq.split('->')][1].split('+')])
		AllSpecies = ([x.split(' ')[-1] for x in [x.strip() for x in [x for x in eq.split('->')][0].split('+')]],[x.split(' ')[-1] for x in [x.strip() for x in [x for x in eq.split('->')][1].split('+')]])		
		NewSpecies = RxnCompare(eq,eq_init)
	if not MB[0]:
...
#		AllSpecies = ([x for x in [x.replace(" ","") for x in eq.split('->')][0].split('+')],[x for x in [x.replace(" ","") for x in eq.split('->')][1].split('+')])
	...
	if min(MB[0]+MB[1]) > 100:
		...
		StchS = MB[0]
		StchP = MB[1]
	return StchS, StchP, AddH, AddH2O, NewSpecies[0], NewSpecies[1], AllSpecies[0], AllSpecies[1], eq, eq_init, TestOfBalance
...
def add_extra_compound (new_compound, lib, time, EF, specialCompounds):
	from Class import compound
	nc = compound('http://www.kegg.jp/dbget-bin/www_bget?cpd:C00080', 'C00080', time, EF, specialCompounds, None)
	nc.AssRxn1 = lambda : ''
	nc.AssRxn2 = lambda : ''
	nc.AssRxn3 = lambda : ''
	nc.CheBI = lambda : ''
