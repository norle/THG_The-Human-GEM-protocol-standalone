...
class DetailedBeta2Stage:
    ...
    def run(self, context: StageContext, work_dir: Path) -> StageResult:
        ...
        if self.id == "load-beta1":
            ...
            if invalid_gprs:
                raise ValueError(
                    "β1 input contains invalid GPRs: " + ", ".join(sorted(invalid_gprs))
                )
        ...
        if self.id == "collect-location-evidence":
            ...
            if isinstance(configured, Mapping):
                for key, value in sorted(
                    ...
                    locations = (
                        sorted(
                            {normalize_location(str(item)) for item in raw_locations}
                        )
        ...
        if self.id == "apply-expansion":
            ...
            save_json_model(model, str(output))
        ...
        if self.id == "export-beta2":
            ...
            model = (
                read_sbml_model(str(model_source))
                if model_source.suffix == ".xml"
                else _load_cobra_model(model_source)
            )
            xml_path = work_dir / "thg-beta2-candidate.xml"
            write_sbml_model(model, str(xml_path))
            ...
            beta1_status = feasibility.get("beta1", {}).get(
                "status", "not-recorded"
            )
            beta2_status = feasibility.get("beta2", {}).get(
                "status", "not-recorded"
            )
...
def detailed_beta2_stages() -> tuple[DetailedBeta2Stage, ...]:
    dependencies = {
        ...
        "resolve-gprs": ("collect-catalysis-evidence",),
        "collect-location-evidence": ("resolve-gprs",),
        "normalize-compartments": ("collect-location-evidence",),
        "infer-complex-and-isoenzyme-locations": ("normalize-compartments",),
