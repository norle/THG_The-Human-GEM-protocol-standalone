...
def deduplicate_gpr(ast: Expression | str) -> GPR:
...
def reduce_gpr(ast_str: str) -> GPR:
    ...
    # Fix numeric gene IDs by prefixing them with 'G' to make them valid
    # identifiers. This prevents ast.Constant errors when gene IDs are pure
...
def sanitize_gpr(gpr: str) -> str:
    ...
    try:
        ...
        if "and" not in lower_gpr and "or" in lower_gpr:
            _LOGGER.info(
                "GPR appears to contain only ORs (no ANDs). "
                "Stripping all brackets for: %r",
                original_gpr,
            )
            gpr = (
    ...
    try:
        ...
        _LOGGER.warning(
            "reduce_gpr failed for GPR %r (after pre-clean): %s -- "
            "falling back by stripping brackets",
            original_gpr,
            e,
        )
