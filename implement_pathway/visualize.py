...
def load_pathway_config(config_path):
    ...
    global PATHWAY_CONFIG, PATHWAY_NAME
...
def visualize_d3js(model_path=None, output_path=None, pathway_config=None):
    ...
    def create_network_data(reactions, metabolite_map, subsystems):
        ...
        for met_id, met_info in metabolite_map.items():
            ...
            key_metabolites = ['ATP', 'ADP', 'GTP', 'GDP', 'H2O', 'Pi', 'H+', 'Phosphate']
            if any(key in met_info['name'] for key in key_metabolites):
                size = 18
            
            # Increase size for proteins and large molecules
            if 'protein' in met_info['name'].lower() or met_info['id'].startswith('MAM100'):
                size = 25
    ...
    def generate_html(nodes, edges, subsystem_colors, compartment_colors, compartment_names, output_path):
        ...
        unique_compartments = sorted(set(n['compartment'] for n in nodes if n['type'] == 'metabolite'))
...
"""
    ...
    reactions, metabolite_map, subsystems = extract_pathway_network(model, pathway_config)
    nodes, edges, subsystem_colors, compartment_colors, compartment_names = create_network_data(reactions, metabolite_map, subsystems)
    generate_html(nodes, edges, subsystem_colors, compartment_colors, compartment_names, output_path)
...
def visualize_plotly(model_path=None, output_path=None, pathway_config=None):
    ...
    if model_path is None:
        model_path = MODEL_PATH if MODEL_PATH else OUTPUT_MODEL_PATH
    if output_path is None:
        output_path = os.path.join(VIZ_OUTPUT_DIR, f'{PATHWAY_NAME}_network_plotly.html')
    ...
    for node in met_nodes:
        ...
        met_hover.append(f"<b>{node_data['name']}</b><br>" +
                        f"ID: {node}<br>" +
    ...
    for node in flexible_rxn_nodes:
        ...
        label = node_data['name'].replace('Transport of ', '').replace(' from cytosol to cytoskeleton', '')
        label = label.replace('Export of ', '').replace(' from cytoskeleton to cytosol', '')
        flex_text.append(label[:25])
        
        flex_hover.append(f"<b>{node_data['name']}</b><br>" +
                         f"ID: {node}<br>" +
    ...
    for node in blocked_rxn_nodes:
        ...
        blocked_hover.append(f"<b>{node_data['name']}</b><br>" +
                            f"ID: {node}<br>" +
                            f"Status: BLOCKED<br>" +
                            f"Min flux: {node_data['min_flux']:.2f}<br>" +
                            f"Max flux: {node_data['max_flux']:.2f}<br>" +
    ...
    print(f'    Flexible reactions: {len(flexible_rxn_nodes)} ({100*len(flexible_rxn_nodes)/len(ck_reactions):.1f}%)')
    print(f'    Blocked reactions: {len(blocked_rxn_nodes)} ({100*len(blocked_rxn_nodes)/len(ck_reactions):.1f}%)')
...
def visualize_graphviz(model_path=None, output_prefix=None, pathway_config=None):
    ...
    dot.attr(rankdir='LR', size='24,12', ratio='compress', dpi='300', ranksep='0.5', nodesep='0.3')
    dot.attr('node', shape='box', style='rounded,filled', fontname='Arial', fontsize='12', width='1.2', height='0.5')
    
    # Metabolite categories
    energy_mets = ['atp_ck', 'adp_ck', 'gtp_ck', 'gdp_ck', 'pi_ck', 'h2o_ck', 'h_ck']
    amino_acids = ['ala__L_ck', 'arg__L_ck', 'asn__L_ck', 'asp__L_ck', 'cys__L_ck', 
                   'gln__L_ck', 'glu__L_ck', 'gly_ck', 'his__L_ck', 'ile__L_ck',
                   'leu__L_ck', 'lys__L_ck', 'met__L_ck', 'phe__L_ck', 'pro__L_ck',
                   'ser__L_ck', 'thr__L_ck', 'trp__L_ck', 'tyr__L_ck', 'val__L_ck']
    proteins = ['actin_protein_ck', 'tubulin_dimer_ck', 'vimentin_protein_ck', 
                'myosin_light_chain_ck', 'actin_complex_ck', 'vimentin_glcnac_ck',
                'myosin_p_ck']
    ...
    for rxn in ck_reactions:
        ...
        if 'demand' in name_lower or rxn.id.startswith('DM_'):
            ...
            if any(aa in name_lower for aa in ['alanine', 'arginine', 'asparagine', 'aspartate', 'cysteine', 'glutamine', 'glutamate', 'glycine', 'histidine', 'isoleucine', 'leucine', 'lysine', 'methionine', 'phenylalanine', 'proline', 'serine', 'threonine', 'tryptophan', 'tyrosine', 'valine']):
            ...
            elif 'h2o' in name_lower or 'h+' in name_lower or 'phosphate' in name_lower or 'adp' in name_lower or 'gdp' in name_lower or 'udp' in name_lower:
        ...
        elif any(kw in name_lower for kw in ['remodel', 'phosphorylation', 'dephosphorylation', 'modification', 'glycosylation', 'glyco']):
    ...
    for rxn in ck_reactions:
        ...
        # Simplified label
        label = rxn.name.replace('Transport of ', '').replace(' from cytosol to cytoskeleton', '')
        label = label.replace('Export of ', '').replace(' from cytoskeleton to cytosol', '')
        label = label.replace(' [cytoskeleton]', '')
        if len(label) > 40:
            label = label[:37] + '...'
        
        dot.node(rxn.id, label, fillcolor=color, shape='box', style=style, fontsize='11')
        
        # Add edges with larger labels
        for met, coeff in rxn.metabolites.items():
            if coeff < 0:  # Substrate
                dot.edge(met.id, rxn.id, label=f'{abs(coeff):.1f}' if abs(coeff) != 1.0 else '', fontsize='10', len='1.0')
            elif coeff > 0:  # Product
                dot.edge(rxn.id, met.id, label=f'{coeff:.1f}' if coeff != 1.0 else '', fontsize='10', len='1.0')
    ...
    with dot.subgraph(name='cluster_legend') as legend:
        ...
        # Metabolites
        legend.node('leg_title', 'Metabolites:', shape='plaintext', fontsize='10', style='')
        legend.node('leg_met', 'Metabolite', shape='ellipse', fillcolor='#E6F3FF', style='filled', fontsize='10')
        ...
        legend.node('leg_title2', 'Reactions (by category):', shape='plaintext', fontsize='10', style='')
        for i, cat in enumerate(categories_present):
            color = category_colors.get(cat, '#BDC3C7')
            legend.node(f'leg_rxn_{i}', cat, shape='box', fillcolor=color, style='filled,bold', fontsize='10')
        
        # Border style
        legend.node('leg_title3', 'Border style:', shape='plaintext', fontsize='10', style='')
        legend.node('leg_style_flex', 'Bold = Flexible', shape='box', fillcolor='#3498DB', style='filled,bold', fontsize='10')
        legend.node('leg_style_blocked', 'Dashed = Blocked', shape='box', fillcolor='#3498DB', style='filled,dashed', fontsize='10')
...
Examples:
  ...
  python3 visualize.py --config config/config_Syndecan1_HS.json --model ../models/endoA_250917_3_with_Syndecan1_HS.json
    ...
    parser.add_argument('option', type=int, nargs='?', default=0,
    ...
    if option == 0:
        ...
        plotly_file = os.path.join(VIZ_OUTPUT_DIR, f'{PATHWAY_NAME}_network_plotly.html')
        ...
        plotly_mtime_before = os.path.getmtime(plotly_file) if plotly_exists_before else 0
        ...
        try:
            ...
            # Check if file was created/updated
            if os.path.exists(plotly_file) and os.path.getmtime(plotly_file) > plotly_mtime_before:
                success_count += 1
        ...
        try:
            ...
            if os.path.exists(png_file) and os.path.getmtime(png_file) > png_mtime_before:
