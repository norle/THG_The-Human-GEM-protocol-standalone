...
try:
    ...
    print(BASE_MODEL_PATH, OUTPUT_MODEL_PATH)
...
def build_compartment_mapping(model, config):
    ...
    for comp_config in config.get("compartments", []):
        ...
        resolved = next(
            (c for c in resolved_compartments if c["name"] == config_name), None
        )
        if resolved:
...
def validate_quick(model_path=None, config_path=None):
    ...
    print(
        f"✓ Loaded: {len(model.metabolites)} metabolites, {len(model.reactions)} reactions"
    )
    ...
    for comp_config in config_compartments:
        ...
        if model_abbrev in model.compartments:
            if config_abbrev != model_abbrev:
                print(
                    f"  ✓ [{config_abbrev}→{model_abbrev}] {comp_name}: {model.compartments[model_abbrev]}"
                )
            else:
                print(
                    f"  ✓ [{model_abbrev}] {comp_name}: {model.compartments[model_abbrev]}"
                )
            ...
            comp_rxns = [
                ...
                if any(m.compartment == model_abbrev for m in r.metabolites)
            ]
    ...
    pathway_name_key = (
        pathway_config.get("pathway_name", "").replace(" ", "_").replace("-", "_")
    )
    pathway_specific_key = f"{pathway_name_key}_specific" if pathway_name_key else None
    pathway_specific_mets = {}

    if pathway_specific_key and pathway_specific_key in pathway_config.get(
        "metabolites", {}
    ):
    ...
    for met_name in sample_targets:
        ...
        for comp_config in config_compartments:
            ...
            comp_met = next(
                (
                    ...
                    if comp_abbrev in m.id and met_name.lower() in m.name.lower()
                ),
                None,
            )
            if comp_met:
        ...
        if found_compartments:
            ...
            print(
                f"  ⚠ {met_name}: not found in new compartments (may only be in cytosol)"
            )
    ...
    if pathway_specific_mets:
        ...
        specific_found = 0
        for met_key, met_data in list(pathway_specific_mets.items())[
            :5
        ]:  # Sample first 5
            ...
            found = next(
                (
                    m
                    for m in model.metabolites
                    if met_name.lower() in m.name.lower()
                    or met_key.lower() in m.name.lower()
                ),
                None,
            )
            if found:
        ...
        print(
            f"  ✓ Found {specific_found}/{min(5, len(pathway_specific_mets))} sampled pathway-specific metabolites"
        )
    ...
    for rxn_config in pathway_config.get("reactions", []):
        ...
        if (
            "demand" in rxn_name.lower()
            or "demand" in subsystem.lower()
            or rxn_id.startswith("DM_")
        ):
    ...
    if not demand_reactions:
        ...
        print(
            "  No demand reactions found in config. Looking for synthesis reactions..."
        )
        for rxn_config in pathway_config.get("reactions", []):
            rxn_id = rxn_config.get("id", "")
            rxn_name = rxn_config.get("name", "")
...
def validate_unit_tests(model_path=None, config_path=None):
    ...
    print(
        f"Test compartments: {test_compartments[:3]}{'...' if len(test_compartments) > 3 else ''}"
    )
    print("=" * 80)
...
def validate_reproducibility(model_path=None, config_path=None):
    ...
    if config_path:
        try:
            ...
            print(f"\nPathway: {pathway_name}")
            print(
                f"Testing reproducibility across {len(pathway_compartments)} compartments and {len(pathway_reaction_ids)} reactions"
            )
    ...
    for run in range(5):
        ...
        if pathway_reaction_ids:
            ...
            pathway_mets = sorted(
                [
                    m.id
                    for m in model.metabolites
                    if m.compartment in pathway_compartments
                ]
            )
        ...
        if pathway_reaction_ids:
            ...
            transport_rxns = sorted(
                [r.id for r in pathway_rxns if "transport" in r.name.lower()]
            )
...
def validate_integration(model_path=None, config_path=None):
    ...
    if config_path:
        try:
            ...
            expected_min_mets = (
                len(cfg.get("metabolites", {}).get("targets", [])) - 5
            )  # Allow some not found
            expected_min_rxns = len(pathway_reaction_ids) - 2  # Allow some missing

            print(f"\nPathway: {pathway_name}")
            print(
                f"Expected: ≥{expected_min_mets} metabolites, ≥{expected_min_rxns} reactions"
            )
    ...
    try:
        ...
        print(
            f"✓ Model loaded: {len(model.metabolites)} metabolites, {len(model.reactions)} reactions"
        )
        passed += 1
    except Exception as e:
        print(f"✗ Failed: {e}")
    ...
    try:
        ...
        matched = sum(
            ...
            if find_metabolite_robust(model_json, met, id_database, test_comp)
        )
        assert matched >= 3
    ...
    try:
        if pathway_reaction_ids:
            found_rxns = sum(
                1 for rxn_id in pathway_reaction_ids if rxn_id in model.reactions
            )
            success_rate = (found_rxns / len(pathway_reaction_ids)) * 100
            assert found_rxns >= expected_min_rxns
            print(
                f"✓ Found {found_rxns}/{len(pathway_reaction_ids)} pathway reactions ({success_rate:.1f}%)"
            )
        else:
    ...
    try:
        ...
        if pathway_reaction_ids:
            ...
            print(
                f"✓ Pathway metabolites: {len(pathway_mets)} (expected ≥{expected_min_mets})"
            )
        else:
            pathway_mets = [
                m for m in model.metabolites if m.compartment in pathway_compartments
            ]
...
def validate_mass_balance(model_path=None, config_path=None):
    ...
    for rxn in pathway_rxns:
        ...
        if any(
            keyword in rxn_id_lower or keyword in rxn_name_lower
...
def validate_synthesis_balance(model_path=None, config_path=None):
    ...
    if config_path:
        try:
            ...
            if synthesis_reactions:
                print(
                    f"Found {len(synthesis_reactions)} synthesis reaction(s) in config"
                )
            else:
                print(
                    "No synthesis reactions found in config (looking for 'Synth' in reaction ID)"
                )
    ...
    for rxn_id, rxn_name in synthesis_reactions.items():
        try:
            ...
            # Check stoichiometry
            substrates = {
                m.id: abs(coeff) for m, coeff in rxn.metabolites.items() if coeff < 0
            }
            products = {
                m.id: coeff for m, coeff in rxn.metabolites.items() if coeff > 0
            }
    ...
    print(
        f"  Result: {'✓ ALL BALANCED' if all_balanced and checked_count > 0 else '✗ IMBALANCED REACTIONS FOUND'}"
    )
    print(f"{'='*80}")
...
def validate_demand_reactions(model_path=None, config_path=None):
    ...
    if config_path:
        try:
            ...
            for reaction in cfg.get("reactions", []):
                rxn_id = reaction["id"]
                if (
                    rxn_id.startswith("DM_")
                    or "demand" in reaction.get("name", "").lower()
                ):
                    rxn_name = reaction.get("name", rxn_id)
    ...
    print(
        f"  Functional: {functional_count} ({100*functional_count/len(demand_reactions):.1f}%)"
    )
    print(
        f"  Blocked: {blocked_count} ({100*blocked_count/len(demand_reactions):.1f}%)"
    )
    if missing_count > 0:
        print(f"  Missing: {missing_count} reactions not found in model")
    ...
    print(
        f"Result: {'✓ ALL DEMANDS FUNCTIONAL' if all_functional else '✗ SOME DEMANDS BLOCKED OR MISSING'}"
    )
    print(f"{'='*80}")
...
def main():
    ...
    if model_path is None:
        if config_path:
            ...
            try:
                ...
                # Get output model path from config
                output_model = cfg.get("model", {}).get("output")
    ...
    for idx, (name, result) in enumerate(results.items(), 1):
        status = "✓ PASSED" if result else "✗ FAILED"
        info = validation_descriptions.get(
            name,
            {
    ...
    if "mass_balance" in results and not results["mass_balance"]:
        ...
        report_lines.append(
            "  Run: python3 validate.py 5 --config <config> --model <model>"
        )
    ...
    if "demand_reactions" in results and not results["demand_reactions"]:
        ...
        report_lines.append(
            "  Run: python3 validate.py 9 --config <config> --model <model>"
        )
    ...
    if "network_topology" in results and not results["network_topology"]:
        report_lines.append(
            "• Network Topology: Review orphaned metabolites and dead-ends"
        )
        report_lines.append(
            "  Run: python3 validate.py 6 --config <config> --model <model>"
        )

    if "quick" in results and not results["quick"]:
        report_lines.append(
            "• Quick Validation: Check FBA constraints and model structure"
        )
        report_lines.append(
            "  Run: python3 validate.py 1 --config <config> --model <model>"
        )

    if passed < total:
        report_lines.append("")
        report_lines.append(
            "For detailed output, re-run individual validation options."
        )
