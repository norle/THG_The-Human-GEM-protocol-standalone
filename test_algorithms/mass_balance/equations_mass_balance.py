...
def atom100(Formula):
    try:
        ...
        return "".join(
            str(i)
            for i in C + H + O + N + P + S + K + Ca + Na + Fe + F + X + M + Se + I + R
        )
    except Exception as e:
        return Formula
...
def inv(A):
    ...
    IM = (
        1 / determ * (MADJ)
    )  # inverse_matrix = 1/det(A)*adjunt_matrix where adjunt_matrix = transposed_cofactor_matrix
    # np.linalg.inv(A)
    return IM
...
def RxnParam2Eq(Reaction, MetList, MetEquiv):
    ...
    if str(Reaction.GTest()) == "1":
        ...
        while p < len(Product):
            ...
            if Product[p][2] in MetEquiv:
                P = (
                    P + " + " + PStch + " " + MetList[MetEquiv[Product[p][2]]].Formula2
                )  # Formula2
            else:
                P = (
...
def MB_LP(eq, AddH, H2O, RxnID):
    ...
    for x in range(0, len(C_subs)):
        locals()["bound%s" % x] = (
            1,
            100,
        )  # previous: (-100,0) corrected as the original
        y += 1
...
def RxnBalance1(eq, RxnID):
    ...
    if len(re.split("\+|->", eq)) < 20:
        ...
        while not MB[0] and i < len(ListOfFunc):
            ...
            if not MB[0] and not re.findall(
                ...
                if not MB[0]:
                    ...
                    print(3)
            if not MB[0] and not re.findall(
                "H2O[^A-Z0-9]|[^A-Z0-9]H2O$|^[H2O][^A-Z0-9]", eq
            ):  # not H2O in the original equation
            ...
            if (
                not MB[0]
                and not re.findall("H[^A-Z0-9]|[^A-Z0-9]H$|^[H][^A-Z0-9]", eq)
                and not re.findall("H2O[^A-Z0-9]|[^A-Z0-9]H2O$|^[H2O][^A-Z0-9]", eq)
            ):
        ...
        AllSpecies = (
            [
                ...
                for x in [x for x in eq.split("->")][0].split("+")
            ],
            [
                re.sub("^[0-9]*", "", x.replace(" ", ""))
                for x in [x for x in eq.split("->")][1].split("+")
            ],
        )
    ...
    if not MB[0]:
        ...
        AddH2O = 0  # In case it is necessary to add H2O, and H2O compound has not been previously added into the model, this flag is used to include it
        AllSpecies = (
            [x for x in [x.replace(" ", "") for x in eq.split("->")][0].split("+")],
            [x for x in [x.replace(" ", "") for x in eq.split("->")][1].split("+")],
        )
        NewSpecies = ["", ""]
        eq = eq
    return (
...
def mass_balance(eq, RxnID):
    ...
    if len(re.split("\+|->", eq)) < 20:
        ...
        if not MB[0]:
            ...
            MEtInEq = [
                x for x in [x.replace(" ", "") for x in eq.split("->")][0].split("+")
            ], [x.replace(" ", "") for x in [x for x in eq.split("->")][1].split("+")]
            IsH = (
                [1 for x in MEtInEq[0] if x in ["H"]],
                [1 for x in MEtInEq[1] if x in ["H"]],
            )
            IsH2O = (
                [1 for x in MEtInEq[0] if x in ["H2O"]],
                [1 for x in MEtInEq[1] if x in ["H2O"]],
            )
